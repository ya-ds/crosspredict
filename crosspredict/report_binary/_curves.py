...
class ByMonthsCurve:
    def _draw_series_to_bar_plot(self, df_series, ax, label="Count"):
        ...
        if df_series.index.dtype == "<M8[ns]":
            plt.xticks(
                ind,
                df_series.index.map(lambda x: x.strftime("%Y-%m")).sort_values(
                    ascending=True
                ),
            )
        else:
...
class DistributionCurve(CurveFabric):
    ...
    def fit(self, df):
        ...
        assert (
            sum(df[self.col_score].isnull()) == 0
        ), f"Column `{self.col_score}` passed in `DistributionCurve` contains None objects"
        log.info(
            f"Using DataFrame where {self.col_score} is not None. DataFrame's shape = {df.shape}"
        )
...
class GenGINICurve(CurveFabric):
    def __init__(
        self, col_score, col_target, col_generation_deals, name=None, **kwargs
    ):
        super().__init__(col_score, col_target, name=name)
    ...
    def fit(self, df):
        ...
        self.series_auc = (
            df[(~df[self.col_score].isnull()) & (~df[self.col_target].isnull())]
            .groupby(self._col_generation_deals)
            .apply(
                lambda x: 100
                * (2 * roc_auc_score(x[self.col_target], x[self.col_score]) - 1)
                if len(x[self.col_target].unique()) > 1
                else None
            )
        )
...
class GenRiskCurve(CurveFabric):
    ...
    def fit(self, df):
        self.risk = df.groupby(self._col_generation_deals)[self.col_target].agg(
            ["count", "mean"]
        )
        return self
    ...
    def plot(self, ax=None, title=None, label=None, name=None, **kwargs):
        ...
        self._draw_series_to_line_plot(
            self.risk["mean"], ax_twinx, label=label, name=name, legend=True
        )
        if title:
            ax.set_title(title, fontsize=14, fontweight="bold")
        return self
...
class MeanProbCurve(CurveFabric):
    ...
    def fit(self, df):
        self.pd_deciles, self.pd_bins = pd.qcut(
            df[self.col_score],
            q=np.arange(0, 1.000001, 0.1),
            duplicates="drop",
            retbins=True,
        )
        self.mean_prob = df.groupby(self.pd_deciles)[self.col_target].agg(
            ["count", "mean"]
        )
        return self
    ...
    def plot(self, ax=None, ax_twinx=None, title=None, label=None, name=None, **kwargs):
        ...
        self._draw_series_to_line_plot(
            self.mean_prob["mean"], ax_twinx, label=label, name=name, legend=True
        )
        if title:
            ax.set_title(title, fontsize=14, fontweight="bold")
        return self
