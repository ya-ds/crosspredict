...
def model_fit(
    ...
    **kwargs,
    ...
    feature_name, cols_exclude, cols_cat, lgb_params = _check_features(
        df, feature_name, cols_exclude, cols_cat, lgb_params
    )
    ...
    model_class = CrossLightgbmModel(
        iterator=iter_df,
...
def forward_selection(
    ...
    **kwargs,
    ...
    feature_name, cols_exclude, cols_cat, _ = _check_features(
        df, feature_name, cols_exclude, cols_cat, None
    )
    ...
    for _ in range(len(feature_name)):
        ...
        for i, f in enumerate(set(feature_name) - set(selected_features)):
            ...
            _, _, cols_cat, _ = _check_features(
                df, current_features, None, cols_cat, None
            )
            model_class = CrossLightgbmModel(
                iterator=iter_df,
        ...
        log.info(
            "{} features left to select ...".format(
                len(set(feature_name) - set(selected_features)) - 1
            )
        )
    ...
    if len(scores_df[scores_df["score_diff_flag"]]) > 0:
        top_features = scores_df[scores_df["score_diff_flag"]].index[0]
    else:
        top_features = len(feature_name)
    top_features = json.dumps(
        {"feature_selection": str(top_features)}, indent=4, sort_keys=True
    )
...
def onefactor(
    ...
    col_target,
    report_directory,
    output_file="README_onefactor.md",
    top_features=20,
):
    logging.basicConfig(level=logging.INFO)
    log = logging.getLogger(__name__)
    ...
    for row in shap_df[:top_features].iterrows():
        ...
        a = a.cat.add_categories(["missing"])
        a = a.fillna("missing")

        fig, _ = plt.subplots()
        fig.set_size_inches(1 * 8, 2 * 5)
        plt.subplots_adjust(
            left=0.125, right=0.9, bottom=0.1, top=0.95, wspace=0.1, hspace=0.3
        )
        ...
        for col in b.columns:
            CurveFabric._draw_series_to_line_plot(
                _, b[col], ax=ax2, label=col, name=col, legend=True
            )
        ...
        for col in b.columns:
            CurveFabric._draw_series_to_line_plot(
                _, b[col], ax=ax4, label=col, name=col, legend=True
            )
...
def model_single_fit(
    ...
    **kwargs,
    ...
    dataset = lgb.Dataset(
        data=df[feature_name], label=df[col_target], categorical_feature=cols_cat
    )
    log.info("feature_num: " + str(len(feature_name)))
    log.info("num_boost: " + str(num_boost))
    log.info(str(lgb_params))
    model = lgb.train(lgb_params, dataset, num_boost_round=num_boost)
