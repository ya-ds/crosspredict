...
class Iterator:
    ...
    def __init__(
        ...
        if cv_byclient:
            assert (
                col_client is not None
            ), "You need provide col_client argument if cv_byclient=True (for RepeatedKFold by client)"
            log.info(f'Using RepeatedKFold by column group "{col_client}"')
            self._repr = f'Using RepeatedKFold by column group "{col_client}"'
        elif col_target is not None:
            log.info(f'Using RepeatedStratifiedKFold by column group "{col_target}"')
            self._repr = f'Using RepeatedStratifiedKFold by column group "{col_target}"'
        else:
            log.info(f"Using RepeatedKFold by all data")
    ...
    def split(self, df):
        ...
        assert self._df_len == len(
            df
        ), "Provided DataFrame doesn't match fitted DataFrame"
