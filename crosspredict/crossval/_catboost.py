...
class CrossCatboostModel(CrossModelFabric):
    ...
    def get_hyperopt_space(
        cls, params: Optional[dict] = None, random_state: int = 0
    ) -> dict:
        result = {
            ...
            "leaf_estimation_iterations": scope.int(
                hp.quniform("leaf_estimation_iterations", 1, 20, 1)
            ),
    ...
    @staticmethod
    def get_dataset(data, label, categorical_feature: List[str], **kwargs) -> Pool:
        if categorical_feature == "auto":
            print(
                "WARNING: auto categorical features detection is not supported."
                "All features is considered as numerical",
                file=sys.stderr,
            )
        ...
        dataset = Pool(
            data=data, label=label, cat_features=categorical_feature, **kwargs
        )
        return dataset
    ...
    def train(
        ...
        **kwargs,
        ...
        if "metric" in params.keys():
            params.pop(
                "metric"
            )  # Catboost haven't param 'metric', 'eval_metric' instead
        ...
        model.feature_name = lambda: _model.feature_names

        if len(model.classes_) == 2:
            model.predict = lambda df, num_iteration: _model.predict(
                df, ntree_end=num_iteration, prediction_type="Probability"
            )[:, 1]
        else:
            model.predict = lambda df, num_iteration: _model.predict(
                df, ntree_end=num_iteration, prediction_type="Probability"
        ...
        evals_result["eval"] = {
            params["eval_metric"]: _model.evals_result_["validation"][
                params["eval_metric"]
