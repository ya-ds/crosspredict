...
class CrossModelFabric(ABC):
    ...
    def fit(self, df):
        ...
        for fold, (train, val) in enumerate(self.iterator.split(df)):
            ...
            dtrain = self.get_dataset(
                data=X_train, label=y_train, categorical_feature=self.cols_cat
            )
            dvalid = self.get_dataset(
                data=X_val, label=y_val, categorical_feature=self.cols_cat
            )
            ...
            if self.valid:
                ...
                if self.params["metric"] in ("auc"):
                    ...
                    scores[fold] = [
                        -1 * x for x in evals_result["eval"][self.params["metric"]]
                    ]
    ...
    def transform(self, df):
        ...
        for fold, (train, val) in enumerate(self.iterator.split(df)):
            ...
            predict.loc[X_val.index] += (
                model.predict(
                    X_val[model.feature_name()], num_iteration=self.num_boost_optimal
                )
                / self.iterator.n_repeats
            )
        return predict
    ...
    def predict(self, test):
        ...
        for fold in self.models.keys():
            ...
            predict += (
                model.predict(
                    test[model.feature_name()], num_iteration=self.num_boost_optimal
                )
                / models_len
            )
    ...
    def shap(self, df: pd.DataFrame, n_samples=500, figsize=(10, 10)):
        ...
        for fold, (train, val) in enumerate(self.iterator.split(df)):
            ...
            shap_df = pd.DataFrame(
                zip(
                    model.feature_name(), np.mean(np.abs(shap_values), axis=0).tolist()
                ),
        ...
        best_features_melt = pd.melt(
            ...
            value_vars=[
                feature
                for feature in best_features.columns.values.tolist()
    ...
    def shap_summary_plot(self, test: pd.DataFrame, n_samples=500):
        ...
        shap_values = self._get_shap_values(explainer, df_sample)
        shap_df = pd.DataFrame(
            zip(model.feature_name(), np.mean(np.abs(shap_values), axis=0)),
